...
$query="select * from purchaing_from_seller where date BETWEEN '$from_date' AND '$to_date' order by id desc";
...
$query="select * from advance_remaining_paid_to_seller_record where paid_date BETWEEN '$from_date' AND '$to_date' order by id desc";
    $row=mysqli_query($connection,$query);
    while($result=mysqli_fetch_array($row))
    { $s_no=$s_no+1;
echo "<tr><td>".$s_no."</td>";
